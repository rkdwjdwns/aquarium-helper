...
def index(request):
    ...
    for tank in page_obj:
        ...
        if latest:
            ...
            ph = latest.ph or 7.0
            if abs(temp - tank.target_temp) >= 2.0:
                status = "DANGER"
                alerts.append("온도 비정상!")
            elif abs(ph - tank.target_ph) >= 0.5:
                status = "WARNING"
                alerts.append("pH 주의!")
...
def apply_recommendation(request):
    try:
        ...
        tank = get_object_or_404(Tank, id=tank_id, user=request.user)
        if 'target_temp' in data: tank.target_temp = data['target_temp']
        if 'target_ph' in data: tank.target_ph = data['target_ph']
        tank.save()
