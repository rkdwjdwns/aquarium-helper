...
def ask_chatbot(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # 2. 시도할 모델 리스트
            models_to_try = [
                "gemini-2.0-flash", # 현재 가장 빠르고 안정적인 모델
                "gemini-1.5-flash",
                "gemini-2.5-flash",
                "gemini-flash-latest",
            ]
            ...
            if "429" in error_msg or "quota" in error_msg.lower():
                friendly_msg = "🐠 AI 물물박사가 잠시 쉬는 시간이에요! 하루 무료 사용량을 다 썼습니다. 잠시 후 다시 시도해 주세요."
            elif "404" in error_msg:
                friendly_msg = "모델을 찾을 수 없습니다. API 키 설정을 확인해 주세요."
            else:
