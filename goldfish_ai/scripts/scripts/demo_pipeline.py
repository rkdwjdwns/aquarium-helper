...
def load_config(path: str = "config.yaml") -> dict:
    ...
    cfg = {
        ...
        # [8] FRS 분석 구간
        "frs_before_sec":     raw.get("analytics", {}).get(
                                  "frs", {}).get("before_sec", 60.0),
        "frs_during_sec":     raw.get("analytics", {}).get(
                                  "frs", {}).get("during_sec", 300.0),
        # 급이 이벤트
...
class TrackFilter:
    ...
    def update(self, fish_ids: list[int], timestamp: float = 0.0):
        ...
        # 대표 ID: 유효 ID 중 프레임 수 상위 expected_fish_count개
        if self._valid:
            sorted_valid = sorted(
                self._valid,
...
class FeatureExtractor:
    ...
    def _zone(self, cy: float, frame_h: int) -> str:
        if cy < frame_h * self.zone_top:   return "TOP"
        elif cy < frame_h * self.zone_bot: return "MID"
        return "BOT"
    ...
    def compute(self, fish_id: int, box: list,
                all_boxes: list, frame_w: int, frame_h: int) -> tuple[dict, bool]:
        x1, y1, x2, y2 = box
        ...
        activity = (
            ...
            if self.speed_buf[fish_id] else 0.0
        )

        frame_area = frame_w * frame_h
        size_index = ((x2-x1)*(y2-y1)/frame_area*100) if frame_area > 0 else 0.0
...
# ─────────────────────────────────────────────────────────────────────────
FISH_METRICS_COLS = [
    "timestamp", "frame_idx", "fish_id",
    "center_x", "center_y", "zone",
    "speed_px_s", "activity", "size_index",
    "overlap_iou", "overlap_count",
    "tracked_duration_sec", "is_representative",   # 논문 코드 반영
    "temperature_c", "ph", "do_mg_l", "turbidity_ntu", "sensor_valid",
]
...
class MetricsWriter:
    ...
    def close(self):
        if self._buf:
            self._w.writerows(self._buf)
        self._f.close()
        print(f"  저장: {self.path}  ({self.total}행 / "
...
def try_load_baseline(baseline_csv: str):
    ...
    """
    import pandas as pd
    from scripts.analytics.activity_pattern import MultiDayActivityAnalyzer, AnalyzerConfig
...
class FRSScheduler:
    ...
    def __init__(self, cfg: dict, writer: MetricsWriter,
...
def draw_overlay(frame, results, features: dict, valid_ids: set,
                 frame_h: int, zone_top: float, zone_bot: float,
                 fps_display: float, writer: MetricsWriter,
    ...
    for ratio, label in [(zone_top, "TOP"), (zone_bot, "MID")]:
        ...
        cv2.putText(frame, label, (4, y-4),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
    cv2.putText(frame, "BOT", (4, h-4),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
        ...
        if ids_t is not None:
            for box_t, tid in zip(results[0].boxes.xyxy, ids_t):
                ...
                if fid not in valid_ids:
                    ...
                    cv2.putText(frame, f"#{fid}(필터중)", (x1, max(y1-6, 12)),
    ...
    cv2.putText(frame, f"FPS:{fps_display:.1f}",
                (w-90, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
    cv2.putText(frame,
                f"rows:{writer.total}  "
                f"spd_drop:{writer.filtered_speed}  "
                f"trk_drop:{writer.filtered_track}",
                (4, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.33, (180, 180, 0), 1)

    # [8] 급이 후 경과시간 표시
    if last_feeding_ts:
        elapsed = time.time() - last_feeding_ts
        cv2.putText(frame, f"급이후: {elapsed:.0f}s",
                    (4, h-18), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 200, 255), 1)
        cv2.putText(frame, "[f]급이 [q]종료",
                    (4, h-4), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (180, 180, 180), 1)
    else:
        cv2.putText(frame, "[f]급이 [q]종료",
                    (4, h-4), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (180, 180, 180), 1)
...
def run(args):
    ...
    # 센서 Reader
    sensor = SensorReader(
        broker = args.broker or cfg["mqtt_broker"],
        topic  = args.topic  or cfg["mqtt_topic"],
    ...
    try:
        while True:
            ...
            results = model.track(
                ...
                conf=cfg["conf"], tracker=cfg["tracker"],
            )
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for box_coords, tid in zip(raw, ids):
                    ...
                    writer.write({
                        ...
                        "fish_id":              fid,
                        **feat,
                        "tracked_duration_sec": track_filter.tracked_duration_sec(fid),
            ...
            # 진행 로그
            if frame_idx % 30 == 0:
                print(f"  {frame_idx:>7}  {len(features):>4}마리  "
            ...
            if args.show:
                vis = draw_overlay(
                    frame_resized.copy(), results, features, valid_ids,
                    frame_h, cfg["zone_top_ratio"], cfg["zone_bottom_ratio"],
                    fps_display, writer, last_feeding_ts,
                )
        ...
        print(f"  트랙 통계: 전체 {tf['total_ids']}ID / "
